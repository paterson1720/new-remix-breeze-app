...
async function generateDocsMenu() {
  const languages = await fs.readdir(`public/documentation`);
  let allVersions = [];

  for (const language of languages) {
    if (language === "__autogenerated__") continue;
    const versions = await fs.readdir(`public/documentation/${language}`);
    allVersions = Array.from(new Set([...allVersions, ...versions]));

    for (const version of versions) {
      const docsMenu = [];

      const filesOrFolders = await fs.readdir(`public/documentation/${language}/${version}`);
      ...
      for (const item of filesOrFolders) {
        ...
        const subFiles = await fs.readdir(`public/documentation/${language}/${version}/${item}`);
        const metaFile = subFiles.find((f) => f === "_meta.md");
        const contentFiles = subFiles.filter((f) => f !== "_meta.md");

        if (metaFile) {
          const filePath = `public/documentation/${language}/${version}/${item}/${metaFile}`;
        ...
        for (const subFile of contentFiles) {
          const filePath = `public/documentation/${language}/${version}/${item}/${subFile}`;
          const md = await fs.readFile(filePath, "utf-8");
      ...
      await fs.writeFile(
        `public/documentation/__autogenerated__/${version}/${language}.json`,
